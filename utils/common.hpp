...
namespace bench
{
    namespace common
    {
        ...
        inline bool is_in_box(const KEY_TYPE &p,const box_t<Dim> &box)
        {
        ...
        template <size_t dim>
        inline void print_points(points_t<dim> &points)
        {
            for (auto p : points)
            {
                print_point(p);
        ...
        template <size_t dim>
        inline void print_knn_result(point_t<dim> &q, points_t<dim> &knn)
        {
        ...
        struct statistics : public boost::geometry::index::detail::rtree::visitor<Value, typename Options::parameters_type, Box, Allocators, typename Options::node_tag, true>::type
        {
            ...
            inline statistics()
                : level(0), levels(1) // count root
                  ,
                  nodes(0), leaves(0), values(0), values_min(0), values_max(0), tree_size(0)
            {
            }
        ...
        inline size_t get_boost_rtree_statistics(Rtree const &tree)
        {
