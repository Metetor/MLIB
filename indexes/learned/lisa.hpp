...
namespace bench
{
    namespace index
    {
        ...
        template <class KEY_TYPE, size_t Dim, size_t BucketSize, size_t Epsilon>
        class LISAInterface : public IndexInterface<KEY_TYPE, Dim>
        {
            using PGMIndex = pgm::PGMIndex<double, Epsilon>;
        ...
        public:
            ...
            inline void print_bucket_to_point(KEY_TYPE &p)
            {
                auto q_result = *(this->_grid_rt->qbegin(bgi::intersects(p)));
                std::cout << "bucket id: " << q_result.second.id << " vol: " << q_result.second.volume << std::endl;
                bench::common::print_box(q_result.first);
            ...
            {
                std::cerr << "Error: knn_query is not supported for LISA Index." << std::endl;
                return {}; // 返回一个空的结果
            }
        ...
        private:
            ...
            inline box_t<Dim> compute_mbr(std::vector<KEY_TYPE> points)
            {
                KEY_TYPE mins, maxs;
                std::fill(mins.begin(), mins.end(), std::numeric_limits<double>::max());
                std::fill(maxs.begin(), maxs.end(), std::numeric_limits<double>::min());
            ...
            inline double compute_volume(box_t<Dim> box)
            {
                auto min_pt = box.min_corner();
                auto max_pt = box.max_corner();
            ...
            inline double compute_volume(box_t<Dim> &box, KEY_TYPE &pt)
            {
                auto min_pt = box.min_corner();
        ...
        // implementation
        template <class KEY_TYPE, size_t Dim, size_t BucketSize, size_t Epsilon>
        void LISAInterface<KEY_TYPE, Dim, BucketSize, Epsilon>::
            build(std::vector<KEY_TYPE> &points)
        {
            ...
            // build rtree for indexing grid buckets
            _grt = new grid_rtree_t(grid_buckets.begin(), grid_buckets.end());

            // sort data by projections and train CDF model
            std::vector<std::pair<KEY_TYPE, double>> point_with_proj;
            point_with_proj.reserve(points.size());
            for (auto &p : points)
            {
                point_with_proj.emplace_back(p, project(p));
        ...
        template <class KEY_TYPE, size_t Dim, size_t BucketSize, size_t Epsilon>
        std::vector<KEY_TYPE> LISAInterface<KEY_TYPE, Dim, BucketSize, Epsilon>::
